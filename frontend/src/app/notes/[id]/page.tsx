...
const NotePage = async ({ params }: PageProps) => {
  ...
  const sampleData: Record<string, Note> = {
    'team-sync-dec-26': {
      get title() { return translateUI("Team Sync - Dec 26"); },
      date: '2024-12-26',
...
- Planned resource allocation for upcoming sprint
    ...
    'product-review': {
      get title() { return translateUI("Product Review"); },
      date: '2024-12-26',
...
- [ ] Review competitor analysis
    ...
    'project-ideas': {
      get title() { return translateUI("Project Ideas"); },
      date: '2024-12-26',
      tags: ['Ideas', 'Planning'],
      content: `
# Project Ideas
...
- Real-time collaboration
    ...
    'action-items': {
      get title() { return translateUI("Action Items"); },
      date: '2024-12-26',
...
- [ ] Setup development guidelines
  ...
  if (!note) {
    return <div className="p-8">{translateUI("Note not found")}</div>;
  }

  return (
    <div className="ink-document min-h-screen bg-card">
      <div className="mb-8">
        <h1 className="font-heading text-[28px] font-semibold mb-4">{note.title}</h1>
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          {note.tags.map((tag) => (
            <div key={tag} className="flex items-center gap-1 bg-accent text-primary px-2 py-1 rounded-full text-sm">
      ...
      <div className="ink-document-body">
