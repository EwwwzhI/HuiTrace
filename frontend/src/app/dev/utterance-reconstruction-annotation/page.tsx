...
type SpeakerInterval = { id: string; start_ms: number; end_ms: number; gt_speaker_key: string; overlap: boolean; uncertain: boolean };
type Bucket = 'clean_single_speaker' | 'speaker_handoff' | 'backchannel' | 'short_speech' | 'true_overlap' | 'noisy_speech' | 'chinese' | 'english' | 'chinese_english_mixed' | 'timing_unavailable_fallback';
type Utterance = { id: string; start_ms: number; end_ms: number; gt_speaker_key: string; boundary_start_uncertain: boolean; boundary_end_uncertain: boolean; contains_backchannel: boolean; overlap: boolean; buckets: Bucket[] };
type Boundary = { id: string; timestamp_ms: number; boundary_kind: 'utterance' | 'speaker_handoff' | 'short_speech'; uncertain: boolean };
...
type ReviewEvidence = { system_label: string; raw_transcripts: { id: string; transcript: string; audio_start_time: number | null; audio_end_time: number | null }[]; v1: { utterances: SystemUtterance[] }; v2: { utterances: SystemUtterance[]; alignment_diagnostics: unknown[]; timing_diagnostics: unknown[] } };
...
export default function UtteranceReconstructionAnnotationPage() {
  ...
  const load = useCallback(async (nextMode: UtteranceAnnotationMode) => {
    try {
      const result = await invoke<View>('api_load_utterance_ground_truth', { request: { artifactPath, groundTruthPath, mode: nextMode } });
      setView(result); setMode(nextMode); setSelectedId(null); setDirty(false); setHistory([]); setFuture([]); setStatus(`Loaded ${nextMode}`);
    } catch (error) { setStatus(String(error)); }
  }, [artifactPath, groundTruthPath]);

  const initialize = useCallback(async () => {
    try {
      await invoke('api_initialize_utterance_ground_truth', { request: { artifactPath, groundTruthPath, datasetSplit: 'calibration' } });
      await load('blind');
    } catch (error) { setStatus(String(error)); }
  }, [artifactPath, groundTruthPath, load]);

  const save = useCallback(async () => {
    if (!gt) return;
    try { await invoke('api_save_utterance_ground_truth', { request: { artifactPath, groundTruthPath, groundTruth: gt } }); setDirty(false); setStatus('Saved'); }
    catch (error) { setStatus(String(error)); }
  ...
  const addBoundary = (kind: Boundary['boundary_kind']) => change(value => ({ ...value, boundaries: [...value.boundaries, { id: `${value.meeting_id}-boundary-${value.boundaries.length + 1}`, timestamp_ms: currentMs, boundary_kind: kind, uncertain: false }] }));
  const undo = () => setHistory(items => { const previous = items.at(-1); if (!previous || !view) return items; setFuture(next => [view.ground_truth, ...next]); setView({ ...view, ground_truth: previous }); setDirty(true); return items.slice(0, -1); });
  const redo = () => setFuture(items => { const next = items[0]; if (!next || !view) return items; setHistory(previous => [...previous, view.ground_truth]); setView({ ...view, ground_truth: next }); setDirty(true); return items.slice(1); });
  ...
  return <main className="min-h-screen space-y-4 bg-background p-5 text-foreground">
    <header><h1 className="text-xl font-semibold">Utterance Reconstruction Ground Truth</h1><p className="text-sm text-muted-foreground">Blind → Review → QA → frozen V1/V2 benchmark. ASR text is read-only.</p></header>
    ...
    {view && <>
      ...
      {mode === 'blind' && <section data-testid="blind-no-system-evidence" className="rounded-xl border border-border bg-card p-4 text-sm">Blind mode: waveform and human Ground Truth only. V1/V2, ASR attribution, alignment and boundary predictions are not loaded.</section>}
      {evidenceVisible && view.review_evidence && <section data-testid="review-system-evidence" className="space-y-3 rounded-xl border border-amber-500/50 bg-card p-4"><h2 className="font-semibold text-amber-600">{view.review_evidence.system_label}</h2><p className="text-sm">V1 utterances: {view.review_evidence.v1.utterances.length} · V2 utterances: {view.review_evidence.v2.utterances.length} · Timing diagnostics: {view.review_evidence.v2.timing_diagnostics.length} · Alignment diagnostics: {view.review_evidence.v2.alignment_diagnostics.length}</p><div className="grid gap-2 md:grid-cols-2"><pre className="max-h-80 overflow-auto rounded bg-muted p-2 text-xs">{JSON.stringify(view.review_evidence.v1.utterances, null, 2)}</pre><pre className="max-h-80 overflow-auto rounded bg-muted p-2 text-xs">{JSON.stringify(view.review_evidence.v2.utterances, null, 2)}</pre></div></section>}
    </>}
  </main>;
}
