...
export const RecordingStatusBar: React.FC<RecordingStatusBarProps> = ({ isPaused = false }) => {
  ...
  return (
    ...
    >
      <div className={`w-2 h-2 rounded-full ${isPaused ? 'bg-warning' : 'bg-seal'}`} />
      <span className={`tabular-nums text-sm ${isPaused ? 'text-warning dark:text-warning' : 'text-foreground'}`}>
        {isPaused ? translateUI("Paused") : translateUI("Recording")} • {formatDuration(displaySeconds)}
      </span>
    </motion.div>
  );
};
