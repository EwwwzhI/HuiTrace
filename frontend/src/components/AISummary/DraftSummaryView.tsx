...
const STATUS_CHIP: Record<BlockStatus, { label: string; className: string }> = {
  draft: { get label() { return translateUI("Draft"); }, className: 'bg-muted text-muted-foreground border-border' },
  approved: { get label() { return translateUI("Approved"); }, className: 'bg-success/10 dark:bg-success/15 text-success dark:text-success border-success/30 dark:border-success/30' },
  edited: { get label() { return translateUI("Edited"); }, className: 'bg-warning/10 dark:bg-warning/15 text-warning dark:text-warning border-warning/30 dark:border-warning/30' },
  rejected: { get label() { return translateUI("Rejected"); }, className: 'bg-destructive/10 dark:bg-destructive/15 text-destructive dark:text-destructive border-destructive/30 dark:border-destructive/30' },
};
...
function ReviewRequiredBanner() {
  ...
  return (
    <div
      role="note"
      aria-label={translateUI("AI-generated content, human review required")}
      className="flex items-start gap-3 p-3 mb-4 bg-warning/10 dark:bg-warning/10 border border-warning/30 dark:border-warning/30 rounded-lg"
    >
      ...
      <div className="text-sm text-warning dark:text-warning">
        <p className="font-semibold">{translateUI("AI-generated · review required")}</p>
        <p className="mt-0.5 text-warning dark:text-warning"> {translateUI("This summary and its action items were generated by AI. Each item is linked to its source transcript segment and must be reviewed and approved by a person before it is finalized.")} </p>
      </div>
    </div>
  );
}
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <button
            ...
            className="inline-flex shrink-0 items-center gap-1 rounded-md border-l-2 border-primary bg-reference px-2 py-1 text-xs text-reference-foreground transition-colors hover:bg-accent"
            aria-label={translateUI("Jump to source transcript segment")}
          >
            <Clock className="h-3 w-3" />
            <span>{translateUI("Source")}</span>
          </button>
        </TooltipTrigger>
        <TooltipContent>{translateUI("Jump to the source transcript segment")}</TooltipContent>
      </Tooltip>
    </TooltipProvider>
  );
}
  ...
  const run = useCallback(
    async (opts: {
      ...
      try {
        ...
        revert();
        toast.error(translateUI("Couldn't apply your change"), {
          get description() { return translateUI("Please try again in a moment."); },
        });
        return false;
      } finally {
        ...
        <div className="flex-1 min-w-0">
          {isEditing ? (
            <textarea
              ...
              className="w-full px-3 py-2 border border-primary/30 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary resize-y"
              autoFocus
            />
          ) : (
            <p
          ...
          <div className="flex items-center flex-wrap gap-x-3 gap-y-1 mt-2">
            ...
            {isEdited && block.original_content !== undefined && (
              <button
                ...
                aria-label={translateUI("Show originally generated text")}
              >
                {showOriginal ? (
                ...
                <span>{translateUI("Original")}</span>
              </button>
            )}
            <SourceLink
          ...
          {isEdited && showOriginal && block.original_content !== undefined && (
            <div className="mt-2 p-2 bg-muted border border-border rounded text-xs text-muted-foreground">
              <span className="font-medium text-muted-foreground"> {translateUI("Originally generated:")}{' '}
              </span>
          ...
          {isRejecting && (
            <input
              ...
              placeholder={translateUI("Why is this wrong? (optional — press Enter to reject)")}
              aria-label={translateUI("Reason for rejecting this block (optional)")}
              className="mt-2 w-full px-3 py-2 border border-destructive/30 dark:border-destructive/40 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-destructive focus:border-destructive"
              autoFocus
            />
          )}
        </div>
          ...
          {isEditing ? (
            <>
              ...
              >
                ...
                <span className="hidden lg:inline">{translateUI("Save")}</span>
              </Button>
              <Button
                variant="outline"
                size="sm"
            ...
            <>
              <Button
                ...
                disabled={isPending}
                title={translateUI("Reject block")}
                aria-label={translateUI("Confirm reject")}
              >
                {isPending ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
                ...
                <span className="hidden lg:inline">{translateUI("Reject")}</span>
              </Button>
              <Button
                variant="ghost"
                size="sm"
                ...
                disabled={isPending}
                title={translateUI("Cancel")}
                aria-label={translateUI("Cancel reject")}
            ...
            <Button
              ...
              title={translateUI("Restore to draft")}
            >
              {isPending ? (
                <Loader2 className="h-4 w-4 animate-spin" />
              ) : (
              ...
              <span className="hidden lg:inline">{translateUI("Restore")}</span>
            </Button>
          ) : (
            <>
              <Button
                ...
                title={translateUI("Approve block")}
                aria-label={translateUI("Approve block")}
              >
                {isPending ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
              ...
              <Button
                ...
                onClick={startEdit}
                disabled={isPending || reviewLocked}
                title={translateUI("Edit block")}
                aria-label={translateUI("Edit block")}
              >
              ...
              <Button
                ...
                disabled={isPending || reviewLocked}
                title={translateUI("Reject block")}
                aria-label={translateUI("Reject block")}
              >
                <X className="h-4 w-4 text-destructive dark:text-destructive" />
              </Button>
            </>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {(item.assignee || item.due) && !isEditing && (
            <div className="flex items-center flex-wrap gap-x-3 gap-y-1 mt-1 text-xs text-muted-foreground">
              {item.assignee && (
                <span>
                  <span className="font-medium">{translateUI("Assignee:")}</span> {item.assignee}
                </span>
              )}
              {item.due && (
                <span>
                  <span className="font-medium">{translateUI("Due:")}</span> {item.due}
                </span>
              )}
            </div>
          )}
          ...
          {isRejecting && (
            <input
              ...
              placeholder={translateUI("Why is this wrong? (optional — press Enter to reject)")}
              aria-label={translateUI("Reason for rejecting this action item (optional)")}
              className="mt-2 w-full px-3 py-2 border border-destructive/30 dark:border-destructive/40 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-destructive focus:border-destructive"
              autoFocus
            />
          )}
          ...
          {isEditing ? (
            ...
            <>
              <Button
                ...
                onClick={confirmReject}
                disabled={isPending}
                title={translateUI("Reject action item")}
                aria-label={translateUI("Confirm reject")}
              >
                {isPending ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
            ...
            <>
              <Button
                ...
                title={translateUI("Approve action item")}
                aria-label={translateUI("Approve action item")}
              >
                {isPending ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
              ...
              <Button
                ...
                onClick={startEdit}
                disabled={isPending}
                title={translateUI("Edit action item")}
                aria-label={translateUI("Edit action item")}
              >
                <Pencil className="h-4 w-4" />
              </Button>
              <Button
                ...
                disabled={isPending}
                title={translateUI("Reject action item")}
                aria-label={translateUI("Reject action item")}
              >
                <X className="h-4 w-4 text-destructive dark:text-destructive" />
              </Button>
            </>
...
export function DraftSummaryView({
  ...
  const approveWholeSummary = async () => {
    ...
    try {
      ...
      if (ok) {
        ...
        toast.success(translateUI("Summary approved"));
        onSummaryApproved?.();
      } else {
        toast.warning(translateUI("Couldn't approve the summary"), {
          get description() { return translateUI("Every block must be approved and its source segment must still exist."); },
        });
      }
    } catch (err) {
      console.error('[DraftSummaryView] approveSummary failed:', err);
      toast.error(translateUI("Couldn't approve the summary"), {
        get description() { return translateUI("Please try again in a moment."); },
      });
    } finally {
    ...
    return (
      <div className="w-full">
        ...
        <div className="flex items-center gap-3 p-4 text-muted-foreground">
          ...
          <span className="text-sm">{translateUI("Loading summary draft…")}</span>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="w-full">
        <ReviewRequiredBanner />
        <div className="p-4 bg-destructive/10 dark:bg-destructive/10 border border-destructive/30 dark:border-destructive/25 rounded-lg">
          <p className="text-sm text-destructive dark:text-destructive font-medium"> {translateUI("Couldn't load the summary draft")} </p>
    ...
    return (
      <div className="w-full">
        ...
        <div className="p-4 bg-muted border border-border rounded-lg text-center">
          <p className="text-sm text-muted-foreground">{translateUI("No summary draft to review yet.")}</p>
          <p className="text-xs text-muted-foreground mt-1"> {translateUI("Generate a summary to see source-linked blocks and action items here.")} </p>
        </div>
      </div>
    );
  }
  ...
  const approveButton = (
    ...
    >
      ...
      {isSummaryApproved ? translateUI("Summary approved") : translateUI("Approve summary")}
    </Button>
  );
  ...
  const exportTrigger = (
    <Button
      ...
      aria-label={translateUI("Export approved summary")}
    >
      {isExporting ? (
        <Loader2 className="h-4 w-4 animate-spin" />
      ) : (
        <Download className="h-4 w-4" />
      )} {translateUI("Export")} <ChevronDown className="h-4 w-4" />
    </Button>
  );
  ...
  return (
    <div className="w-full">
        ...
        <div className="text-sm text-muted-foreground">
          {isSummaryApproved ? (
            <span className="inline-flex items-center gap-1 text-success dark:text-success font-medium">
              <Check className="h-4 w-4" /> {translateUI("Approved")} </span>
          ) : (
            <span>
              {nonRejected.filter((b) => b.status === 'approved').length} {translateUI("of")}{' '}
              {nonRejected.length} {translateUI("block(s) approved")} </span>
          )}
        </div>
          ...
          {isSummaryApproved ? (
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent>{translateUI("Approve the summary to export")}</TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
          {approveGateReason ? (
      ...
      {isSummaryApproved && excludedActionItemCount > 0 && (
        <p className="text-xs text-warning dark:text-warning mb-4">
          {excludedActionItemCount} {translateUI("action item")} {excludedActionItemCount === 1 ? '' : 's'} {translateUI("not yet approved — not included in the export.")} </p>
      )}
      ...
      <div className="space-y-4">
        ...
        {/* Action items */}
        {hasActionItems && (
          <SectionCard icon={ListChecks} title={translateUI("Action items")} count={actionItems.length} aiLabel>
            <div className="divide-y divide-border">
